...
function drawOutfoxxed(ctx, facing) {
  ...
  const hair = '#5b4231';
  const dark = '#302723';
  const highlight = '#92704d';
  const frame = '#a9b4b2';
  torso(ctx, '#454a40', skin, '#292f2d');
  rect(ctx, 12, 28, 12, 10, '#7aa2f7');
  poly(ctx, [[7, 27], [13, 29], [11, 37], [7, 36]], '#30362f');
  poly(ctx, [[25, 27], [22, 31], [24, 38], [29, 36]], '#555a4b');
  poly(ctx, [[5, 8], [9, 3], [15, 1], [21, 2], [26, 6], [29, 12], [30, 18], [28, 24], [29, 28], [25, 32], [19, 31], [13, 33], [7, 31], [3, 26], [2, 20], [4, 14]], dark);
  poly(ctx, [[6, 8], [12, 3], [19, 2], [24, 6], [27, 11], [27, 18], [26, 24], [27, 29], [23, 31], [15, 29], [8, 31], [4, 26], [5, 20], [4, 15]], hair);
  if (facing === 'back') {
    poly(ctx, [[15, 3], [11, 10], [8, 17], [9, 24], [6, 29], [6, 22], [5, 17], [9, 10], [12, 4]], highlight);
    poly(ctx, [[20, 4], [24, 10], [23, 17], [26, 24], [24, 29], [22, 23], [21, 16], [22, 10], [18, 5]], '#79583e');
    rect(ctx, 15, 12, 1, 12, '#735039');
  } else if (facing === 'profile') {
    poly(ctx, [[18, 7], [23, 9], [25, 14], [25, 18], [29, 21], [26, 23], [25, 28], [21, 31], [17, 28], [14, 21], [15, 13]], skin);
    rect(ctx, 19, 9, 4, 5, light);
    rect(ctx, 20, 14, 8, 8, frame);
    rect(ctx, 21, 15, 6, 6, skin);
    rect(ctx, 23, 17, 3, 2, '#444b49');
    rect(ctx, 14, 15, 7, 1, frame);
    rect(ctx, 21, 15, 5, 1, '#d0dbd4');
    rect(ctx, 22, 25, 4, 1, '#9e7062');
    rect(ctx, 21, 29, 3, 1, '#a18675');
    poly(ctx, [[14, 5], [18, 4], [18, 10], [15, 17], [16, 24], [13, 28], [12, 22], [13, 15], [11, 10]], hair);
    rect(ctx, 13, 10, 1, 8, highlight);
  } else {
    poly(ctx, [[12, 7], [20, 7], [24, 11], [25, 19], [24, 26], [21, 30], [17, 32], [12, 30], [8, 26], [7, 19], [9, 12]], skin);
    rect(ctx, 13, 9, 8, 4, light);
    rect(ctx, 8, 20, 2, 5, shade);
    rect(ctx, 24, 19, 1, 6, shade);
    rect(ctx, 7, 14, 9, 8, frame);
    rect(ctx, 18, 14, 10, 8, frame);
    rect(ctx, 8, 15, 7, 6, '#d9b099');
    rect(ctx, 19, 15, 8, 6, '#d9b099');
    rect(ctx, 15, 16, 4, 1, frame);
    rect(ctx, 9, 15, 5, 1, '#d0dbd4');
    rect(ctx, 20, 15, 6, 1, '#d0dbd4');
    rect(ctx, 10, 17, 4, 2, '#e5e9ff');
    rect(ctx, 21, 17, 4, 2, '#e5e9ff');
    rect(ctx, 11, 17, 2, 2, '#3f4848');
    rect(ctx, 22, 17, 2, 2, '#3f4848');
    rect(ctx, 16, 18, 2, 5, shade);
    smile(ctx, 12, 25, 11, '#a57365');
    rect(ctx, 16, 30, 5, 1, '#a58a78');
    rect(ctx, 12, 29, 1, 1, '#a58a78');
    poly(ctx, [[17, 3], [20, 5], [15, 11], [10, 15], [7, 19], [5, 22], [5, 17], [7, 11], [12, 6]], hair);
    poly(ctx, [[13, 5], [16, 4], [12, 10], [8, 14], [6, 18], [7, 12]], highlight);
    poly(ctx, [[22, 7], [25, 10], [27, 16], [26, 23], [28, 27], [25, 29], [24, 24], [25, 18], [23, 12]], '#79583e');
  }
}
