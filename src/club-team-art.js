...
export function makeTeamCharacter(npc, facing = 'smile') {
  ...
  if (['coat', 'hoodie', 'overshirt', 'blazer'].includes(p.style)) {
    rect(ctx, 14, 33, 7, 6, p.trim); rect(ctx, 29, 33, 7, 6, p.trim);
    rect(ctx, 21, 34, 7, 32, '#24283b');
  ...
  if (p.lanyard) {
    ...
    rect(ctx, 21, 54, 7, 10, C.paper); rect(ctx, 22, 57, 5, 3, '#315a78');
  }
  ...
  rect(ctx, 17, 5, 8, 2, p.hairHighlight || '#6b5545'); rect(ctx, 29, 8, 4, 2, p.hairHighlight || '#6b5545');
  ...
  if (p.stubble) { rect(ctx, 16, 27, 4, 2, '#9f826b'); rect(ctx, 31, 26, 2, 3, '#9f826b'); rect(ctx, 20, 30, 11, 1, '#9f826b'); }
  if (p.chinBeard) rect(ctx, 23, 29, 5, 3, p.hair);
  ...
  if (p.glasses) {
    ...
    if (p.frames) {
      ctx.strokeStyle = '#565348'; ctx.lineWidth = 1; ctx.strokeRect(14.5, 16.5, 9, 6); ctx.strokeRect(26.5, 16.5, 9, 6); rect(ctx, 24, 18, 2, 1, '#565348');
    }
  }
  rect(ctx, p.wideSmile ? 19 : 21, 27, p.wideSmile ? 12 : 9, 2, '#e5ddc8');
  if (facing === 'profile') {
...
export function commandBoard(crew, title = 'OMARCHY SECURITY', detail = 'PENTEST · 0-DAY · DEFENSE · FORENSICS · REVERSE') {
  ...
  text(ctx, title, 36, 58, 44, C.green);
  text(ctx, detail, 36, 99, 23, C.cyan);
  crew.forEach((npc, i) => {
    const x = 36 + i * 198;
    ctx.drawImage(makeTeamCharacter(npc), x + 52, 124, 48, 104);
